...
const CUSTOMER_DETAILS_QUERY = /* GraphQL */ `
  query CustomerDetails($customerAccessToken: String!) {
    customer(customerAccessToken: $customerAccessToken) {
      ...
      membership_profile: metafield(namespace: "custom", key: "membership_profile_v1") {
        value
      }
...
export async function getProducts(first = 24) {
  ...
  const products = edges.map(({ node }) => {
    ...
    const variantIdByColor = {};
    const variantPriceByColor = {};
    const variantMaxQtyByColor = {};
...
export async function getCustomerDetails(customerAccessToken) {
  ...
  try {
    const rawMp = customer?.membership_profile?.value;
    if (rawMp) membershipProfile = JSON.parse(rawMp);
  } catch {

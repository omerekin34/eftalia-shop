...
async function fetchJudgeMeReviews(productNumericId, productHandle = "") {
  ...
  try {
    ...
    console.log("[JudgeMe][fetchReviews][summary]", {
      cleanId: cleanId || null,
      handle: handle || null,
      count: mapped.length,
    });
...
export async function getProduct(handle) {
  ...
  console.log("[Shopify][getProduct][reviewsMeta]", {
    handle,
...
export async function setCustomerTicketMetafield(customerAccessToken, key, tickets) {
  if (!customerAccessToken) throw new Error("Yetkilendirme bulunamadı.");
  const value = JSON.stringify(Array.isArray(tickets) ? tickets : []);
  const data = await storefrontFetch(CUSTOMER_UPDATE_MUTATION, {
    customerAccessToken,
    customer: {
      metafields: [
        {
